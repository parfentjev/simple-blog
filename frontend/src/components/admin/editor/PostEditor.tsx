...
const PostEditor: FC = () => {
  ...
  const handleSave = async (event: FormEvent) => {
    ...
    if (!token) toast.error('Not authenticated.')

    if (updateDate) postState.date = new Date()
  ...
  useEffect(() => {
    if (deleteCounter < 3 || !postState.id) return
  ...
  return (
    <form onSubmit={handleSave} className="editor-form">
      ...
      <div className="post-options">
        ...
        <input type="checkbox" id="updateDate" onChange={() => setUpdateDate((update) => !update)} />
